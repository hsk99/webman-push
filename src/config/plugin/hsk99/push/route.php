 ...
 * 推送js客户端文件
 */
Route::any('/plugin/hsk99/push/push.js', function (Request $request) {
    return response()->file(base_path() . '/vendor/hsk99/webman-push/src/push.js');
});

/**
 * 私有频道鉴权，这里应该使用session辨别当前用户身份，然后确定该用户是否有权限监听channel_name
 */
Route::any(config('plugin.hsk99.push.app.auth'), function (Request $request) {
    $pusher = new Api(config('plugin.hsk99.push.app.api'), config('plugin.hsk99.push.app.app_key'), config('plugin.hsk99.push.app.app_secret'));
 ...
 */
Route::any(parse_url(config('plugin.hsk99.push.app.channel_hook'), PHP_URL_PATH), function (Request $request) {
    ...
    $expected_signature = hash_hmac('sha256', $body, config('plugin.hsk99.push.app.app_secret'), false);
